/*
Sampler program - runs a thread to collect light samples and display output to terminal
*/
...
void Sampler_shutdownRoutine()
{
    ...
    led_shutdownRoutine();
    pthread_cleanup_push(free_mutexes, &historyMutex);
...
static void *samplerThread()
{
    ...
    while (running)
    {
        ...
        if (getTimeInMs() >= currentTime + 1000)
        {
            ...
            else if (recentDips != dipCount && dipCount!=0)
            {
                secondsWithDips++;
            }
            recentDips = dipCount;
            ...
            else
            {
                ...
                {
                    printf("%5.3f    ", recent[index%arraySize]);
                }
