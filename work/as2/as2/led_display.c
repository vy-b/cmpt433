/* 
Program to display number on LED in a background thread.
Run from main program along with sampler and udp listener threads.
Code adapted from Dr. Brian's guide
*/
#include <stdio.h>
#include <stdbool.h>
#include <unistd.h>
...
static int* digitLeft;
static int* digitRight;
static int led_init();
void led_reset(int i2cFileDesc);
...
void* ledThread()
{
    ...
    led_reset(i2cFileDesc);
    return NULL;
}
void displayNum(int i2cFileDesc, int num)
{
    digitLeft = (int*) malloc(2*sizeof(int));
    getDigitValues(num/10, digitLeft);
    digitRight = (int*) malloc(2*sizeof(int));
...
void led_reset(int i2cFileDesc)
{
    ...
    writeToFile("0", RIGHT_VALUE);
    writeToFile("0", LEFT_VALUE);
    free_malloc(digitLeft);
    free_malloc(digitRight);
    close(i2cFileDesc);
}
static void turnOnLeft(int i2cFileDesc, int *digit) {
    // turn on left
